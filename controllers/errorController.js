...
HandleNoIDError = (err) => {
  ...
  return new AppError(message, 400, "E0001");
};

HandleTokenExpiry = (err) => {
  return new AppError("Your token is expired", 401, "E0003");
};
  ...
  if (err.isOperational) {
    if (!err.message) {
      return res.status(statusCode).json({
        status: status,
        message:
          "No Data Found With That ID Or Please check whether you have inserted the token or clear your cache",
      });
    }
    return res.status(statusCode).json({
      status: status,
